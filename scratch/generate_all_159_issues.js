...
const rawText = `Governments should place few, if any, restrictions on scientific research and development. 
...
// Parse into exactly 159 pairs of prompt and directions
...
while (i < lines.length) {
  let promptLines = [];
  // Accumulate lines until we see "Write a response..."
  while (i < lines.length && !lines[i].startsWith('Write a response')) {
    promptLines.push(lines[i]);
    i++;
  }
  let directions = '';
  if (i < lines.length && lines[i].startsWith('Write a response')) {
    directions = lines[i];
    i++;
  }
  ...
  if (prompt) {
    const padId = String(topicId).padStart(3, '0');
    topics.push({
      id: topicId,
      accessCode: `ISSUE${padId}`,
      prompt: prompt,
      directions: directions || 'Write a response in which you discuss your views on the statement/claim.',
      category: prompt.startsWith('Claim:') ? 'Claim & Reason Task' : 'Issue Essay Task'
    });
    topicId++;
  }
}

console.log(`Extracted ${topics.length} total topic items!`);

const content = `export interface GreIssueTopic {
  id: number;
  prompt: string;
  directions: string;
  accessCode: string;
...
export const OFFICIAL_GRE_ISSUE_POOL: GreIssueTopic[] = ${JSON.stringify(topics, null, 2)};
`;

fs.writeFileSync(path.join(__dirname, '../src/services/greIssuePool.ts'), content, 'utf8');
console.log('Successfully generated greIssuePool.ts with ALL 159 topics!');
